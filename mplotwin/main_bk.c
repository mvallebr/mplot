...
int main(int argc, char *argv[]){
  mplot *m;
  FIXED_NUMBER x[NUMITEMS], y[NUMITEMS], z[NUMITEMS], fex[NUMITEMS], fey[NUMITEMS], temp;
//  int exp1, exp2;
  int i;
#ifdef USEFILE
  FILE *f, *f2;
  ...
  do {
   ...
   //Update the screen
   } while (mplot_update(m, x, y, z,NUMITEMS, x, y, NUMITEMS, METHOD_BARNES, 10, MPLOT_AUTODETECT, MPLOT_GRAPHTYPE_SHADE));
...
//    fey[i]*=(FIXED_NUMBER)pow((double)10, (double)exp2);
  ...
  while (mplot_update(m, x, y, z, num_items, x, y, num_items, METHOD_BARNES, 10, MPLOT_AUTODETECT, MPLOT_GRAPHTYPE_SHADE)); return -1;
  fclose(f2);
#endif //USEFILE
